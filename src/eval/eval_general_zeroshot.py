...
# Set up logging, specify log file path
# logging.basicConfig(
#     level=logging.DEBUG,
#     format='%(asctime)s - %(levelname)s - %(message)s',
#     filename='evaluation.log',  # Specify log filename
#     filemode='1'  # 'w' means write mode, will overwrite log file on each run
# )
...
class ModelEvaluator:
    def __init__(
        ...
        # Question template
        self.question_template = "{Question} Answer with the option's letter from the given choices directly."
    ...
    def evaluate(self):
        ...
        while i < total_samples:
            try:
                ...
                inputs = {
                    k: v.to(device) if isinstance(v, torch.Tensor) else v  # Move only Tensor types
    ...
    def get_per_token_logps(self, model, input_ids):
        ...
        Reference implementation from grpo_trainer.py
        Args:
...
def evaluate(model_name,dataset_name,batch_size):
    ...
    output_path = os.path.join(".", "logs", dataset_name, "test", base_path, f"{os.path.basename(model_path)}_{dataset_name}_eval_zeroshot.json")
